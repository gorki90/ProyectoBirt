...
export class FormularioPrincipalComponent implements OnInit {
  ...
  selectedAttributes!: Arbol; // Para almacenar los atributos seleccionados (si estamos editando)
  ...
  //errorres de validaciones backend
  errorBackend!: string;
  constructor(
...
tasks: this.fb.array(
  (this.selectedAttributes?.tasks || []).map(task =>
    this.fb.group({
      ...
      .subscribe({
        next: data => {
          ...
          if (!this.isEditMode) {
            ...
            this.atributoService.newTreeCoordinates.subscribe({
              next: coordinates => {
              ...
              error: err => {
                // Capturamos el error y pasamos los mensajes al ErrorService
                if (err.error.errors) {
                  let rutaReturn = 'mapaid';
                  const errorMessages = Object.values(err.error.errors); // Extraemos los mensajes de error del backend
                  this.errorService.setError(errorMessages as string[], rutaReturn); // Pasamos los mensajes de error al ErrorService
                } else {
                  this.errorService.setError(['Ocurrió un error inesperado al obtener las coordenadas del árbol']);
                }
              }
            });
          } else {
            ...
            this._id.getId
              ...
              .subscribe({
                next: data => {
                  this.arbolidEdit = data;
                  console.log('id88888', data);
                    ...
                    .subscribe({
                      next:attributes => {
                        if (attributes) {
                          ...
                          console.log("idarbol0", attributes.id);
                      ...
                      error: err => {
                        ...
                        if (err.error.errors) {
                          ...
                          this.errorService.setError(errorMessages as string[],rutaReturn); // Pasamos los mensajes de error al ErrorService
                            ...
                            this.errorService.setError(['Ocurrió un error inesperado al cargar los atributos del árbol']);
                        }
                      }
    ...
    if (!this.isEditMode) {
      ...
      this.wfsService.createTree(formData)
        ...
        .subscribe({
          ...
          error: err => {
            console.log('errorgrande', err);
            // Capturamos el error y pasamos los mensajes al ErrorService
              if (err.error.errors) {
                let rutaReturn='/formulario-definitivo';
                const errorMessages = Object.values(err.error.errors); // Extraemos los mensajes de error del backend
                this.errorService.setError(errorMessages as string[], rutaReturn); // Pasamos los mensajes de error al ErrorService
              } else {
                  this.errorService.setError(['Ocurrió un error inesperado al crear el árbol']);
              }
            }
        })
    } else {
          ...
          this.wfsService.updateTree(formData, this.arbolidEdit)
          .pipe(takeUntil(this.unsubscribe$)) // Cancela la suscripción en OnDestroy
          .subscribe({
            next: response => {
            ...
            error: err => {
              ...
              if (err.error.errors) {
                ...
                this.errorService.setError(['Ocurrió un error inesperado al actualizar los atributos del árbol']);
              }
            }
          });
    };
