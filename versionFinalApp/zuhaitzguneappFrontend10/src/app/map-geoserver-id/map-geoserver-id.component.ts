...
import { ArbolGeoserver } from '../modelo/arbolGeoserver.model';
...
export class MapGeoserverIdComponent implements OnInit {
  ...
  private attributes!:ArbolGeoserver;
  ...
  ngOnInit(): void {
    ...
    // Usar el evento 'postrender' para asegurarse de que el mapa esté listo
    this.map.once('postrender', () => {
      // Esperar a que el control de atribuciones se haya insertado
      setTimeout(() => {
        // Seleccionar el contenedor del control de atribución usando CSS
        const attributionElement = document.querySelector('.ol-attribution')  as HTMLElement;
        if (attributionElement) {
          attributionElement.style.position = 'absolute';
          attributionElement.style.bottom = '0px';
          attributionElement.style.right = '10px';
          attributionElement.style.zIndex = '1000';
          
        }
        // Personalizar el contenedor del control de zoom
        const zoomElement = document.querySelector('.ol-zoom') as HTMLElement;
          if (zoomElement) {
            zoomElement.style.width = 'auto'; // Hacer que el control de zoom no ocupe toda la fila
            zoomElement.style.height = 'auto'; 
            zoomElement.style.position = 'absolute';
            zoomElement.style.top = '10px';
            zoomElement.style.left = '10px';
          }
        
        // Personalizar el contenedor del control de escala
        const scaleElement = document.querySelector('.ol-scale-line') as HTMLElement;
        const scaletext = document.querySelector('.ol-scale-line-inner') as HTMLElement;
          if (scaleElement) {
            scaleElement.style.width = 'auto'; // Hacer que el control de escala no ocupe toda la fila
            scaleElement.style.height = '10px'; // Ajustar la altura 
            scaleElement.style.transform = 'rotate(180deg)'; // Rotar la barra de escala
            scaletext.style.transform = 'rotate(180deg)'; // Rotar el texto de la escala
            scaleElement.style.position = 'absolute';
            scaleElement.style.top = '20px';
            scaleElement.style.right = '10px';
            scaleElement.style.marginLeft = '10px';
            scaletext.style.marginTop = '5px';
            scaleElement.style.color = 'black';
            // Configuración del estilo para la barra de escala
            scaleElement.style.background = 'repeating-linear-gradient(90deg, #000 0%, #000 50%, #fff 50%, #fff 100%)';
            scaleElement.style.backgroundSize = '20px 100%'; // Ajusta el tamaño de cada banda
            scaleElement.style.padding = '5px';    
            // Asegura que el fondo se vea correctamente
            scaleElement.style.borderRadius = '4px';    // Esquinas redondeadas
            scaleElement.style.border = '2px solid #000';
    ...
    this.map.on('singleclick', (evt) => {
      ...
      this.map.forEachFeatureAtPixel(evt.pixel, (feature) => {
        ...
        if (feature) {
          ...
          this.attributes = feature.getProperties() as ArbolGeoserver;
