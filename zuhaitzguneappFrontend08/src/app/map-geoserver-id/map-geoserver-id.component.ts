...
//    const extent = [-2.9345523651976593, 43.245006495955934, -2.9344212673108827, 43.24508829095301]; // valores reales del encuadre nativo 
    const extent = [-2.9355678650923065, 43.26479958314269, -2.9249662816327917, 43.26381857957066]; // valores reales del encuadre nativo 
    ...
    this.loadFeatures(); // Cargar las caracterÃ­sticas al iniciar
    this.setupClickHandler();
  }
  // Alternar el modo de creaciÃ³n
  toggleCreateMode(): void {
    this.isCreateMode = !this.isCreateMode;
    if (this.isCreateMode) {
      this.isEditMode = false; // Desactivar el modo de ediciÃ³n
    }
  }
  toggleEditMode(): void {
    this.isEditMode = !this.isEditMode;
    if (this.isEditMode) {
      this.isCreateMode = false; // Desactivar el modo de creaciÃ³n
    }
  }
  // MÃ©todo para cargar las caracterÃ­sticas usando el servicio
  private loadFeatures(): void {
    // obtengo los datos del geoserver
    this.wfsService.getFeatures().subscribe((data) => {
      const features = new GeoJSON().readFeatures(data, {
        featureProjection: 'EPSG:3857', // ProyecciÃ³n del mapa
      });
      (this.wfsLayer.getSource() as VectorSource).addFeatures(features); // AÃ±adir las caracterÃ­sticas al mapa
    });
  }
...
//          console.log("Atributos del Ã¡rbol:", this.attributes)
          console.log("locationData:", this.attributes)
        }
      });
      if (!this.isEditMode && !this.isCreateMode && this.attributes) {
        this.attributeService.updateAttributes(this.attributes);
        this.router.navigate(["/lista"]);
//        this.router.navigate(['locationData'])
      }
      // En modo de ediciÃ³n, revisamos si se hizo clic en un Ã¡rbol para editarlo
...
//            this.router.navigate(['locationData'])
      ...
      // Si estamos en modo de ediciÃ³n y no se hizo clic en un Ã¡rbol, iniciamos el proceso de creaciÃ³n
      if (this.isCreateMode && !featureClicked) {
        ...
        this.router.navigate(['/formulario-definitivo']); // Navegar al formulario de creaciÃ³n
//        this.router.navigate(['locationData'])
      }
    });
  }
