...
export class Datos_menuComponent implements OnInit {
  ...
  setSection(section: string) {
    ...
    @Output() formSelected1 = new EventEmitter<void>();
  @Output() formSelected2 = new EventEmitter<void>();
  @Output() formSelected3 = new EventEmitter<void>();
  @Output() formSelected4 = new EventEmitter<void>();

    emitirSection(section: String) {
    if (section == 'locationData')
      this.formSelected1.emit();
    if (section == 'dasometricData')
      this.formSelected1.emit();
    if (section == 'healthStatus')
      this.formSelected1.emit();
    if (section == 'tasks')
      this.formSelected1.emit();
...
/*
  ...
  coordinates: [number, number] | null = null; // Para las coordenadas si estamos en modo de creaciÃ³n
  selectedAttributes: any; // Para almacenar los atributos seleccionados (si estamos editando)
  private coordinatesSubscription!: Subscription; // AÃ±adir variable para la suscripciÃ³n
        ...
        this._editMode.getEditMode
          ...
          .subscribe({
            next: data => {
              ...
              // Si estamos creando, obtener las coordenadas del Ã¡rbol
              if (!this.isEditMode) {
                // Suscribirse al servicio de coordenadas, asegurÃ¡ndose de manejar la suscripciÃ³n correctamente
                ...
                // Modo de ediciÃ³n: cargar los atributos seleccionados en el formulario
                  ...
                  .subscribe(attributes => {
                    if (attributes) {
                      ...
                      if (Array.isArray(attributes.tasks) && attributes.tasks.length > 0) {
                          ...
                          // Si no hay tareas, asegÃºrate de que el FormArray estÃ© vacÃ­o
                          console.log("No hay tareas, el FormArray se vacÃ­a");
  ...
  onSubmit() {
    ...
    if (!this.isEditMode) {
      ...
      this.wfsService.createTree(formData)
        ...
        .subscribe({
          next: response => {
            console.log('Ãrbol creado correctamente', response);
            // Redirijo despuÃ©s de la creaciÃ³n exitosa
        ...
        .subscribe({
          next: data =>{
            ...
            this.wfsService.updateTree(formData, data)
              ...
              .subscribe({
              next: response => {
                console.log('Ãrbol actualizado correctamente', response);
                // Redirigir o hacer algo despuÃ©s de la creaciÃ³n exitosa
                this.routes.navigate(['/mapaid']);
                },
  ...
  // metodo para marcar la seccion del formulario en la que se estÃ¡
  setSection(section: string) {
    this.currentSection = section;
  }
